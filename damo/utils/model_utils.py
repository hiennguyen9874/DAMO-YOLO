...
def get_model_info(model, tsize):
    ...
    head = model.head
    h, w = tsize
    img = torch.randn((1, 3, stride, stride), device=next(model.parameters()).device)
    ...
    total_flops = 0
    info = ""
    for name, flops, params, latency in zip(
        ("backbone", "neck", "head"), (bf, nf, hf), (bp, np, hp), (bl, nl, hl)
    ):
        params /= 1e6
        flops /= 1e9
        ...
        total_flops += flops
        info += (
            f"{name}'s params(M): {params:.2f}, "
            + f"flops(G): {flops:.2f}, latency(ms): {latency*1000:.3f}\n"
        )
    info += (
        f"total latency(ms): {total_latency*1000:.3f}, " + f"total flops(G): {total_flops:.2f}\n"
    )
    return info
