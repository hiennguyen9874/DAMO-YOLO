...
from ..losses.gfocal_loss import DistributionFocalLoss, GIoULoss, QualityFocalLoss
...
class ZeroHead(nn.Module):
    ...
    def _build_not_shared_convs(self, in_channel, feat_channels):
        ...
        for i in range(self.stacked_convs):
            ...
            cls_convs.append(
                ConvBNAct(
                    chn, feat_channels, kernel_size, stride=1, groups=1, norm="bn", act=self.act
                )
            )
            reg_convs.append(
                ConvBNAct(
    ...
    def _init_layers(self):
        ...
        self.gfl_cls = nn.ModuleList(
            [
                nn.Conv2d(self.feat_channels[i], self.cls_out_channels, 3, padding=1)
                for i in range(len(self.strides))
            ]
        )
        ...
        self.gfl_reg = nn.ModuleList(
            [
                nn.Conv2d(self.feat_channels[i], 4 * (self.reg_max + 1), 3, padding=1)
                for i in range(len(self.strides))
            ]
        )
    ...
    def loss(
        ...
        cls_reg_targets = self.get_targets(
            cls_scores, decoded_bboxes, gt_bboxes, mlvl_center_priors, gt_labels_list=gt_labels
        )
        ...
        if len(pos_inds) > 0:
            ...
            logger.info(
                f"No Positive Samples on {bbox_preds.device}! May cause performance decrease. loss_bbox:{loss_bbox:.3f}, loss_dfl:{loss_dfl:.3f}, loss_qfl:{loss_qfl:.3f} "
            )
    ...
    def get_target_single(
        ...
        assign_result = self.assigner.assign(
            cls_scores.detach(), center_priors, bbox_preds.detach(), gt_bboxes, gt_labels
        )
