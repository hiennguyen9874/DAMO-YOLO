...
def im_detect_bbox_aug(model, images, device, config):
    ...
    boxlists_i = im_detect_bbox(
        model, images, config.testing.input_min_size, config.testing.input_max_size, device, config
    )
    ...
    for scale in config.testing.augmentation.scales:
        ...
        boxlists_scl = im_detect_bbox_scale(model, images, scale, max_size, device, config)
        add_preds_t(boxlists_scl)

        if config.testing.augmentation.scales_hflip:
            boxlists_scl_hf = im_detect_bbox_scale(
                model, images, scale, max_size, device, config, hflip=True
            )
    ...
    for boxlist in boxlists:
        results.append(
            filter_results(
                boxlist, config.model.head.num_classes, config.testing.augmentation.nms_thres
            )
        )
...
def im_detect_bbox(model, images, target_scale, target_max_size, device, config):
    ...
    transform = T.Compose(
        [
            ...
            T.ToTensor(),
            T.Normalize(
                mean=config.dataset.input_pixel_mean,
...
def im_detect_bbox_hflip(model, images, target_scale, target_max_size, device, config):
    """
...
def im_detect_bbox_scale(model, images, target_scale, target_max_size, device, config, hflip=False):
    """
    ...
    if hflip:
        boxlists_scl = im_detect_bbox_hflip(
            model, images, target_scale, target_max_size, device, config
        )
    else:
        boxlists_scl = im_detect_bbox(model, images, target_scale, target_max_size, device, config)
    return boxlists_scl
