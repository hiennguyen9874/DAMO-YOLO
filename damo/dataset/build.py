...
def build_dataset(cfg, ann_files, is_train=True, mosaic_mixup=None):
    ...
    if not isinstance(ann_files, (list, tuple)):
        raise RuntimeError("datasets should be a list of strings, got {}".format(ann_files))
    datasets = []
...
def make_batch_sampler(
    dataset, sampler, images_per_batch, num_iters=None, start_iter=0, mosaic_warpper=False
):
...
def build_dataloader(
    ...
    ), "training_imgs_per_batch ({}) must be divisible by the number " "of GPUs ({}) used.".format(
        batch_size, num_gpus
    )
    ...
    for dataset in datasets:
        ...
        batch_sampler = make_batch_sampler(
            dataset, sampler, images_per_gpu, num_iters, start_iter, enable_mosaic_mixup
        )
    ...
    if is_train:
        assert len(data_loaders) == 1, "multi-training set is not supported yet!"
