...
class Trainer:
    def __init__(self, cfg, args, tea_cfg=None, is_train=True):
        ...
        if tea_cfg is not None:
            ...
            self.feature_loss = FeatureLoss(
                self.model.neck.out_channels, self.tea_model.neck.out_channels, distiller="cwd"
            ).to(self.device)
        else:
            self.distill = False
            self.grad_clip = None

        self.optimizer = self.build_optimizer(cfg.train.momentum, cfg.train.weight_decay)
        # resume model
        ...
        if self.cfg.train.ema:
            logger.info("Enable ema model! Ema model will be evaluated and saved.")
        ...
        self.setup_iters(
            iters,
            self.start_epoch,
            cfg.train.total_epochs,
            cfg.train.warmup_epochs,
            cfg.train.no_aug_epochs,
        ...
        self.lr_scheduler = cosine_scheduler(
            cfg.train.base_lr_per_img,
            cfg.train.batch_size,
            cfg.train.min_lr_ratio,
            self.total_iters,
            self.no_aug_iters,
            self.warmup_iters,
            cfg.train.warmup_start_lr,
        )
    ...
    def get_data_loader(self, cfg):
        ...
        train_dataset = build_dataset(
            cfg, cfg.dataset.train_ann, is_train=True, mosaic_mixup=cfg.train.augment.mosaic_mixup
        )
        val_dataset = build_dataset(cfg, cfg.dataset.val_ann, is_train=False)

        iters_per_epoch = math.ceil(
            len(train_dataset[0]) / cfg.train.batch_size
        )  # train_dataset is a list, however,
    ...
    def train(self, local_rank):
        ...
        for data_iter, (inps, targets, ids) in enumerate(self.train_loader):
            ...
            if self.distill:
                ...
                distill_weight = (
                    (1 - math.cos(cur_iter * math.pi / len(self.train_loader))) / 2
                ) * (0.1 - 1) + 1
            ...
            if self.grad_clip is not None:
                nn.utils.clip_grad_norm_(
                    self.model.parameters(), max_norm=self.grad_clip, norm_type=2
                )  # for stable training
    ...
    def save_ckpt(self, ckpt_name, local_rank, update_best_ckpt=False):
        if local_rank == 0:
            ...
            ckpt_state = {
                ...
                "feature_loss": self.feature_loss.state_dict() if self.distill else None,
            }
            save_checkpoint(
                ckpt_state,
                update_best_ckpt,
    ...
    def evaluate(self, local_rank, val_ann):
        ...
        for idx, dataset_name in enumerate(val_ann):
            output_folder = os.path.join(self.output_dir, self.exp_name, "inference", dataset_name)
            if local_rank == 0:
                mkdir(output_folder)
